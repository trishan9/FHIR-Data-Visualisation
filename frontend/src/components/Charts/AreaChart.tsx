...
const AreaChart: React.FC<{ data: CareData[] }> = ({ data }) => {
  ...
  useEffect(() => {
    ...
    const parsedData = data.map((d) => ({
      ...
      value: d?.total?.value ?? 0,
    }));

    // Aggregate data by month
    const aggregatedData = d3.rollup(
      parsedData,
      (v) => d3.mean(v, (d) => d.value), // You can change aggregation method as needed
    ...
    const parentHeight = 450; // Set your desired overall height here
    const margin = { top: 50, right: 50, bottom: 50, left: 50 }; // Adjust margins as needed
    ...
    svg
      ...
      .attr("fill", "rgba(26,143,251, 0.3)") // Translucent blue fill
      .attr("stroke", "#1A8FFB") // Blue stroke
    ...
    tooltip
      ...
      .attr("stroke", "black")
      .attr("stroke-width", 1)
      .attr("stroke-opacity", 0.3) // Set opacity to 0.5 or any desired value
